...
class Mondrian {
  ...
  saveOriginalPositions() {
    this.originalRects = JSON.parse(JSON.stringify(this.rects));
  }

  // recover position
  restoreOriginalPositions() {
    this.rects = JSON.parse(JSON.stringify(this.originalRects));
  }
  ...
  applyRandomOffset() {
    this.rects.forEach(rect => {
      // just little rect
      if (rect.gw === 1 && rect.gh === 1) {
        // const rect
        const offsetX = random(-2, 2) / UNIT;
        const offsetY = random(-2, 2) / UNIT;
  ...
  draw() {
    // Draw rect with mouse interaction
    for (const r of this.rects) {
      // Calculate center point of the rectangle
      const centerX = (r.gx + r.gw / 2) * UNIT;
      const centerY = (r.gy + r.gh / 2) * UNIT;
      
      // Calculate distance from mouse to rectangle center
      const distance = dist(mouseX, mouseY, centerX, centerY);
      
      // Calculate scale factor based on distance
      // The further away, the bigger the scale (max scale: 1.5, min scale: 0.8)
      const maxDistance = dist(0, 0, canvasSize, canvasSize);
      ...
      // Apply texture effects after drawing the scaled rectangle
      if (r.color === RED || r.color === BLUE) {
        this.drawOilPaintEffect(r, scaleFactor);
      } else if (r.color === YELLOW) {
        this.drawCrayonEffect(r, scaleFactor);
      }
    }
  }

  // oil red and blue rect
  drawOilPaintEffect(rect, scaleFactor) {
    ...
    drawingContext.shadowBlur = 8 * scaleFactor;
    drawingContext.shadowColor = color(red(rect.color), green(rect.color), blue(rect.color), 60);
   
   
    this.drawOilBrushStrokes(x, y, w, h, rect.color, scaleFactor);
   
    drawingContext.shadowBlur = 0;
  }

  // Draw oil color
  drawOilBrushStrokes(x, y, w, h, color, scaleFactor) {
    const strokeCount = max(2, (w * h) / 200);
   
    for (let i = 0; i < strokeCount; i++) {
      const strokeX = x + random(w);
      const strokeY = y + random(h);
      const strokeW = random(3, 8) * scaleFactor;
      const strokeH = random(2, 4) * scaleFactor;
  ...
  // Yellow rect
  drawCrayonEffect(rect, scaleFactor) {
    const x = rect.gx * UNIT;
    const y = rect.gy * UNIT;
    const w = rect.gw * UNIT;
    const h = rect.gh * UNIT;
    ...
    // DrawCrayonStreaks
    this.drawCrayonStreaks(x, y, w, h, scaleFactor);
  }

  // DrawCrayonStreaks
  drawCrayonStreaks(x, y, w, h, scaleFactor) {
    ...
    for (let i = 0; i < streakCount; i++) {
      ...
      const streakWidth = random(3, 6) * scaleFactor;
...
function keyPressed() {
  // space key
  if (key === ' ') {
    mondrian.applyRandomOffset();
    return false; // stop
  }
  
  // R key
  if (key === 'r' || key === 'R') {
    mondrian.restoreOriginalPositions();
    return false; // stop
  }
}