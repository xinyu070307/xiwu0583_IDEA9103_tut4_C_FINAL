...
// 马赛克参数
const MOSAIC_SIZE = 10; // 马赛克格子大小（像素）
...
class Mondrian {
  ...
  draw() {
    // LET CANVASIZE
    let tempBuffer = createGraphics(canvasSize, canvasSize);
    tempBuffer.rectMode(CORNER);
    tempBuffer.noStroke();
    
    // DRAW PICTURE
    for (const r of this.rects) {
      tempBuffer.fill(r.color);
      tempBuffer.rect(r.gx * UNIT, r.gy * UNIT, r.gw * UNIT, r.gh * UNIT);
    }
    
    // APPLY EFFECT
    this.applyMosaicEffect(tempBuffer);
  }

  // 
  applyMosaicEffect(sourceBuffer) {
    // CONST SIZE
    const mosaicCols = Math.ceil(canvasSize / MOSAIC_SIZE);
    const mosaicRows = Math.ceil(canvasSize / MOSAIC_SIZE);
    
    // DRAW MOSAIC
    for (let col = 0; col < mosaicCols; col++) {
      for (let row = 0; row < mosaicRows; row++) {
        const x = col * MOSAIC_SIZE;
        const y = row * MOSAIC_SIZE;
        
        // ENSURE COLOR
        const centerX = x + MOSAIC_SIZE / 2;
        const centerY = y + MOSAIC_SIZE / 2;
        
        // SIZE
        if (centerX < canvasSize && centerY < canvasSize) {
          const pixelColor = sourceBuffer.get(centerX, centerY);
